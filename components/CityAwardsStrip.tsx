...
export function CityAwardsStrip() {
  ...
  }, [controls, paused, reduceMotion, shiftPx]);
          ...
          {reduceMotion ? (
            ...
            >
              {marqueeAwards.map((award, i) => (
                <AwardCard key={`${award.title}-${award.year}-${i}`} award={award} />
              ))}
            </motion.ul>
          )}
        </div>
